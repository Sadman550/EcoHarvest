...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['action'])) {
        ...
        $package_id_input = filter_input(INPUT_POST, 'package-id', FILTER_SANITIZE_STRING);
        $package_id = $package_id_input ? "PKG$package_id_input" : '';
        ...
        if (empty($bid)) $errors[] = "Batch ID is required.";
        if (empty($package_id_input) || !preg_match("/^[0-9]+$/", $package_id_input)) $errors[] = "Packaging ID must be numeric.";

        // Check for duplicate bid or package_id
        $stmt = $conn->prepare("SELECT id FROM packaging WHERE (bid = ? OR package_id = ?) AND id != ?");
        if ($stmt) {
            $edit_id = isset($_POST['edit_id']) ? intval($_POST['edit_id']) : 0;
            $stmt->bind_param("ssi", $bid, $package_id, $edit_id);
            $stmt->execute();
            $result = $stmt->get_result();
            if ($result->num_rows > 0) {
                $errors[] = "Batch ID or Packaging ID already exists.";
            }
            $stmt->close();
        }
        ...
        if (empty($errors)) {
            if ($_POST['action'] == 'add') {
                $stmt = $conn->prepare("INSERT INTO packaging (type, weight, date, bid, package_id) VALUES (?, ?, ?, ?, ?)");
                if ($stmt) {
                    $stmt->bind_param("sdsss", $type, $weight, $date, $bid, $package_id);
                    if ($stmt->execute()) {
                ...
                $stmt = $conn->prepare("UPDATE packaging SET type = ?, weight = ?, date = ?, bid = ?, package_id = ? WHERE id = ?");
                if ($stmt) {
                    $stmt->bind_param("sdsssi", $type, $weight, $date, $bid, $package_id, $edit_id);
                    if ($stmt->execute()) {
...
// Fetch available Batch IDs from quality_analysis, excluding those already in packaging
$batch_ids_result = $conn->query("SELECT qa.bid, qa.weight FROM quality_analysis qa 
                                 LEFT JOIN packaging p ON qa.bid = p.bid 
                                 WHERE p.bid IS NULL 
                                 ORDER BY qa.bid");
        ...
        <div class="d-flex justify-content-between align-items-center w-100">
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                ...
                <ul class="navbar-nav flex-row">
                        ...
                        <li class="nav-item"><a href="packaging.php" class="nav-link active"><i class="fas fa-box-open me-1"></i> Packaging</a></li>
      ...
      <table class="table table-hover align-middle">
        ...
        <tbody id="tableBody">
          ...
          <tr class="animate__animated animate__fadeIn" style="animation-delay: <?php echo ($index * 0.05); ?>s;">
            ...
            <td><span class="badge bg-light text-dark"><?php echo htmlspecialchars($item['bid']); ?></span></td>
            <td><span class="badge bg-light text-dark"><?php echo htmlspecialchars($item['packageId']); ?></span></td>
        ...
        <form id="packagingForm" method="POST" action="">
          ...
          <div class="row mb-3">
            <div class="col-md-6 mb-3 mb-md-0">
              <label for="bid" class="form-label">Batch ID</label>
              <select id="bid" name="bid" class="form-select" onchange="autofillWeight()" required>
                <option value="" selected disabled>Select Batch ID</option>
                <?php while ($batch = $batch_ids_result->fetch_assoc()): ?>
                  <option value="<?php echo htmlspecialchars($batch['bid']); ?>" 
                          data-weight="<?php echo htmlspecialchars($batch['weight']); ?>">
                    <?php echo htmlspecialchars($batch['bid']); ?>
                  </option>
                <?php endwhile; ?>
              </select>
            </div>
          ...
          <div class="row mb-3">
            <div class="col-md-6 mb-3 mb-md-0">
              ...
              <div class="input-group">
                <input type="number" step="0.01" id="weight" name="weight" class="form-control" readonly required aria-describedby="weightUnit">
          ...
          <div class="mb-3">
            <label for="package-id" class="form-label">Packaging ID</label>
            <div class="input-group">
              <span class="input-group-text">PKG</span>
              <input type="text" id="package-id" name="package-id" class="form-control" placeholder="e.g. 001" required pattern="[0-9]+" aria-describedby="packageIdPrefix">
            </div>
          </div>
...
function editPackaging(id) {
  ...
  document.getElementById("package-id").value = item.packageId.replace('PKG', '');
...
function updateTable(filteredItems = packagingInfo) {
  ...
  paginatedItems.forEach((item, index) => {
    ...
    row.innerHTML = `
      ...
      <td><span class="badge bg-light text-dark">${item.bid}</span></td>
      <td><span class="badge bg-light text-dark">${item.packageId}</span></td>
      <td class="text-center action-buttons">
